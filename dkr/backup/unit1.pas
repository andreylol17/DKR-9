...
begin
  SQLQuery1.Close;
  SQLQuery1.SQL.Text:='select * from Cars';
  SQLQuery1.Open;
  SQLQuery2.Close;
  SQLQuery2.SQL.Text:='select * from Cars';
  SQLQuery2.Open;
...
begin
  ...
  SQLQuery1.SQL.Text := 'insert into cars (brand, model, engine, price) values(:b,:m,:e,:p);';  // добавляем sql запрос для добавления данных
  ...
  SQLite3Connection1.DatabaseName := 'C:\Users\andre\OneDrive\Рабочий стол\dkr\school.db'; // указывает путь к базе
...
begin
  ...
  fEdit.eEngine.Text:= SQLQuery1.Fields.FieldByName('engine').AsString;
  ...
  if not (fEdit.eEngine.Text= '') then SQLQuery1.Fields.FieldByName('engine').AsString := fEdit.eEngine.Text;
  if not (fEdit.eModel.Text= '') then SQLQuery1.Fields.FieldByName('model').AsString := fEdit.eModel.Text;
...
begin
  ...
  SQLQuery1.SQL.Text := 'select * from Cars where model = :m'; // добавляем наш запрос
  SQLQuery1.ParamByName('m').AsString:= eSearch.text;// присваиваем требуемый параметр
  SQLQuery1.Open;// открываем запрос
  end
  else ShowMessage('Введите модель мотоцикла для поиска записи!');
  DBGrid1.Columns.Items[0].Width:=30;
  DBGrid1.Columns.Items[1].Width:=120;
  DBGrid1.Columns.Items[2].Width:=180;
  DBGrid1.Columns.Items[3].Width:=120;
